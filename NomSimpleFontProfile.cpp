...
const atl::color NomSimpleFontProfile::pcsm_defaultColor = atl::color(0.f, 0.f, 0.f, 0.f);

NomSimpleFontProfile::NomSimpleFontProfile(float in_edge1,
...
void NomSimpleFontProfile::colorLerp(atl::color in_color)
{
    for(auto & l_color : m_colors)
        l_color.interpolate(atl::color(in_color.r, in_color.g, in_color.b, 1.f), in_color.a);
}

void NomSimpleFontProfile::colorMult(atl::color in_color)
{
    for(auto & l_color : m_colors)
    {

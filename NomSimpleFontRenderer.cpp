...
NomSimpleFontRenderer::NomSimpleFontRenderer(const NomFiles & in_files,
    ...
    pm_rendererState(in_rendererState)
{
    ...
    const GLsizei l_dynamicDataVertexStride = sizeof(NomSimpleFontRenderer::Vertex);
...
#define BUFFER_OFFSET(i) ((char *)NULL + (i))
    ...
    for(int idx_buffer = 0; idx_buffer < pcsm_numBuffers; idx_buffer++)
    {
        ...
        glVertexAttribPointer(ATTRIBUTE_VERT_TEX_COORD, 2, GL_FLOAT, GL_FALSE, l_dynamicDataVertexStride, (const GLvoid*)offsetof(class NomSimpleFontRenderer::Vertex, u));

        glEnableVertexAttribArray(ATTRIBUTE_VERT_INDEX);
        glVertexAttribPointer(ATTRIBUTE_VERT_INDEX, 1, GL_FLOAT, GL_FALSE, l_dynamicDataVertexStride, (const GLvoid*)offsetof(class NomSimpleFontRenderer::Vertex, index));
    }
...
#undef BUFFER_OFFSET
    ...
    if (!RenderUtil::compileShader(in_files, &l_vertShader, GL_VERTEX_SHADER, "SimpleFont"))
    {
    ...
    if (!RenderUtil::compileShader(in_files, &l_fragShader, GL_FRAGMENT_SHADER, "SimpleFont"))
    {
...
NomSimpleFontRenderer::~NomSimpleFontRenderer()
{
...
void NomSimpleFontRenderer::render(const NomSimpleFontProfile & in_profile,
                                   ...
                                   int in_numLines)
{
            ...
            if(l_inTag)
            {
                if(*l_str == '>')
                {
                    if(l_tagString[0] == 's' && l_tagString[1] == ':')
                    {
                        ...
                        for(int idx_scaleChar = 2; idx_scaleChar < l_tagString.length(); idx_scaleChar++)
    ...
    // Now draw string:
    {
        NomSimpleFontRenderer::Vertex * l_vertexData = pm_vertexData;
        NomSimpleFontRenderer::Vertex * l_end = pm_vertexData + atl::c_array_len(pm_vertexData);
...
void NomSimpleFontRenderer::p_drawBufferUsingProfile(const NomSimpleFontProfile & in_profile,
                                                     ...
                                                     unsigned int in_numPrims)
{
    ...
    // Upload vertex data:
    glBufferSubData(GL_ARRAY_BUFFER, 0, atl::c_array_len(pm_vertexData) * sizeof(NomSimpleFontRenderer::Vertex), pm_vertexData);
...
atl::point2f NomSimpleFontRenderer::drawCharacter(const NomSimpleFontProfile & in_profile,
                                                  const NomFont & l_font,
